...
trait WithDistributionFormulaByRestrictions extends WithProgresive {
  ...
  case class LinearRestriction(
      distanceFromReference: Double,
      progresive: Double
  )
  val linearRestricctions: List[LinearRestriction] = restrictions
    .map(r => LinearRestriction(lengthToPoint(r.elementPoint), r.progresive))
    .sortBy(_.distanceFromReference)
  assert(
    areCloseInLinearReference(linearRestricctions.head.distanceFromReference, 0)
  )
  assert(
    areCloseInLinearReference(
      linearRestricctions.last.distanceFromReference,
      length
    )
  )
  ...
  override def calcProgresive(ep: ElementPoint): Double = {
    ...
    val closestRestrictionToDx = linearRestricctions.find(lr =>
      areCloseInLinearReference(lr.distanceFromReference, dx)
    )
    if (closestRestrictionToDx.isDefined) {
      closestRestrictionToDx.get.progresive
    } else {
      val segmentResultOpt =
        linearRestricctions.zip(linearRestricctions.tail).find {
          case (m, n) =>
            m.distanceFromReference <= dx && dx <= n.distanceFromReference
        }
...
trait ConverterWithDistrutionFormula[A <: WithRestrictionsIncremental[
  A,
  _
], C <: WithProgresive] {
...
object WithDistributionFormula {
  implicit val r2f_RectConverter: ConverterWithDistrutionFormula[
    RectSegmentRestrictions,
    RectSegmentProgresiva
  ] =
    (value: RectSegmentRestrictions) =>
      value.element match {
        case r: TRectSegment =>
          RectSegmentProgresiva(r.originPoint, r.endPoint, value.restrictions)
      }
  implicit val r2f_CircConverter: ConverterWithDistrutionFormula[
    CircleSegmentRestrictions,
    CircleSegmentProgresiva
  ] =
    (value: CircleSegmentRestrictions) =>
      value.element match {
  ...
  implicit val r2f_FaintConverter: ConverterWithDistrutionFormula[
    FaintSegmentRestrictions,
    FaintSegmentProgresiva
  ] =
    (value: FaintSegmentRestrictions) =>
      value.element match {
        case f: TFaintElement =>
          FaintSegmentProgresiva(f.from, f.end, value.restrictions)
      }

  def convert[A <: WithRestrictionsIncremental[A, _], C <: WithProgresive](
      value: A
  )(implicit conv: ConverterWithDistrutionFormula[A, C]): C =
    conv.convert(value)

}

trait TRectSegmentProgresiva
    extends TRectSegment
    with WithDistributionFormulaByRestrictions {
...
trait TCircleSegmentProgresiva
    extends TCircleSegment
    with WithDistributionFormulaByRestrictions {
  override def findPointByLength(lengthParam: Double): Option[ElementPoint] = {
    if (lengthParam < 0 || lengthParam > length) {
      None
      ...
      Some(
        ElementPoint(
          centerPoint + ((in.point - centerPoint) << beta),
          None,
          this
        )
      )
...
trait TFaintSegmentProgresiva
    extends TFaintElement
    with WithDistributionFormulaByRestrictions {
  override def findPointByLength(length: Double): Option[ElementPoint] = None
}
trait TEfficientSeqEjeElementsProgresiva
...
case class RectSegmentProgresiva(
    originPoint: TPoint,
    endPoint: TPoint,
    restrictions: List[Restriction]
) extends TRectSegmentProgresiva

case class CircleSegmentProgresiva(
    originPoint: TPoint,
    centerPoint: TPoint,
    endPoint: TPoint,
...
case class FaintSegmentProgresiva(
    from: TPoint,
    end: TPoint,
    restrictions: List[Restriction]
