...
class TEfficientSeqEjeElementsProgresivaTest extends AnyFlatSpec {
  ...
  it should "generate kml file" in {
    ...
    val resultPath = "/home/vmchura/Documents/001.Projects/vevial/ejevialview/src/test/resources/testTramo.kml"
    val result = xmlBuiler.toEje.map(_.exportKML(resultPath))
    result match {
      case Right(_) =>
        assert(java.nio.file.Files.exists(new io.File(resultPath).toPath))
      case Left(error) =>
        println(error)
