...
class ActorCriticSoftmaxAgentLinear() extends BaseAgent {
  ...
  var tc: LinearTileCoder =  null//new PendulumTileCoder(iht_size = agentInfo.iht_size,num_tilings = num_tilings,num_tiles = agentInfo.num_tiles)
  ...
  override def agent_init(agentInfo: AgentInfo): Unit = {
    ...
    tc =  new LinearTileCoder(iht_size = agentInfo.iht_size,num_tilings = num_tilings,num_tiles = agentInfo.num_tiles)
