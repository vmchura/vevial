...
case class SimpleExperiment(override val id: UUID, position: Int = 1, finished: Boolean = false) extends Experiment{
  ...
  override def update(action: Int): Experiment =
    if(!finished)
      action match {
        case 1 => if(position == 1) copy(finished = true) else copy(position = position-1)
        case 2 => this
        case 3 => copy(position = position +1, finished = position == 9)
      }
    else
      this
  ...
  override def regard(action: Int): Double =
    if(!finished)
      action match {
        case 1 => if(position == 1) -100 else -2
        case 2 => -1
        case 3 => 1
