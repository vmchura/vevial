 ...
 * @author Jarek Sacha
 */
object SurveyModifierForm extends JFXApp3 {

  def start(): Unit = {

    val resource = getClass.getResource("/SurveyModifier.fxml")

    if (resource == null) {
      throw new IOException("Cannot load resource: SurveyModifier.fxml")
    }
