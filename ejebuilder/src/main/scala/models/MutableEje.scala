...
import io.vmchura.vevial.PlanarGeometric.BasicEje.{
  ELementBlocksContainer,
  EfficientSeqEjeElements,
  PointsOnElementGenerator,
  TSeqEjeElementsBase
}
...
class MutableEje(val elements: List[TEjeElement]) extends TSeqEjeElementsBase {
  require(elements.nonEmpty, "MutableEje: elements are empty")
  ...
  override def append(o: TSeqEjeElementsBase): TSeqEjeElementsBase =
    throw new NotImplementedError("cant append")
  ...
  override val rightmostPoint: TPoint =
  ...
  private val (blockX, blockY) = {
    val bx =
      new ELementBlocksContainer(leftmostPoint.x, rightmostPoint.x, 12, _.x)
  ...
  override def projectPoint(point: TPoint): Option[ElementPoint] = {
    ...
    EfficientSeqEjeElements.bruteForceCalculation(
      intersection.toList,
      point
    )
