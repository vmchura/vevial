...
import io.vmchura.vevial.elementdata.{
  CrudeIRIData,
  IRIElementData,
  TElementWithPoint,
  UPoint
...
/**
  ...
  */
class RelevamientoIRI[T <: TElementWithPoint[T]: ClassTag](
    override val elements: Seq[T],
    val header: List[Array[String]],
    val interval: Int
) extends TSimpleRelevamiento[T] {

  override def sliceBy(
      minX: Double,
      maxX: Double,
      minY: Double,
      maxY: Double
    ...
    new RelevamientoIRI(
      ...
      header :+ Array(
        s"Solo considerando puntos en el rango Este-Oeste: [$minX:$maxX] Norte-Sur: [$minY:$maxY]"
      ),
      interval
    )
  }
...
object RelevamientoIRI {
  def apply(
      elements: Seq[IRIElementData],
      header: List[Array[String]],
      interval: Int
  ): RelevamientoIRI[IRIElementData] =
    new RelevamientoIRI(elements, header, interval)

  def apply[T <: TElementWithPoint[T]: ClassTag](
      source: File,
      builder: CrudeIRIData => T
  ): RelevamientoIRI[T] = {

    import scala.jdk.CollectionConverters._

    val reader = new CsvListReader(
    ...
    val interval: Int =
      header.find(_.exists(_.contains("Processing interval:"))) match {
        case Some(line) =>
          val indxPI = line.indexWhere(_.contains("Processing interval:"))
          val digitsOnly = line(indxPI + 1).filter(_.isDigit)
          if (digitsOnly.isEmpty)
            throw new IllegalArgumentException(
              "Incorrecto archivo, no es un .CSV válido/Not defined interval number"
            )
          else
            digitsOnly.toInt
        case None =>
          throw new IllegalArgumentException(
            "Incorrecto archivo, no es un .CSV válido/Not defined interval line"
          )
      }
    ...
    dataElements.zipWithIndex.foreach {
      case (iriElement, indx) =>
        if (indx > 0)
          dataElements.update(
            indx,
            iriElement.withPrevElement(dataElements(indx - 1))
          )
        if (indx + 1 < n)
          dataElements.update(
            indx,
            iriElement.withNextElement(dataElements(indx + 1))
          )
